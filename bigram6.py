...
eval_interval = 300
learning_rate = 3e-4 # Inserting a single self-attention block to our network: 4
device = 'cuda' if torch.cuda.is_available() else 'cpu' # New things Number 1: GPU
eval_iters = 200
n_embd = 384 # Minor code cleanup: 2
n_head = 6 # Layernorm : 3
n_layer = 3 # Layernorm : 3
dropout = 0.2 # Layernorm : 4
# ------------
...
class Head(nn.Module): # Inserting a single self-attention block to our network: 1
    ...
    def __init__(self, head_size):
        ...
        self.dropout = nn.Dropout(dropout) # Layernorm : 6
    ...
    def forward(self, x):
        ...
        wei = self.dropout(wei) # Layernorm : 6
...
class MultiHeadAttention(nn.Module): # Multi-headed self-attention: 1
    ...
    def __init__(self, num_heads, head_size):
        ...
        self.proj = nn.Linear(n_embd, n_embd) # Residual connections: 4
        self.dropout = nn.Dropout(dropout) # Layernorm : 4

    def forward(self, x):
        #return torch.cat([h(x) for h in self.heads], dim=-1)
        out = torch.cat([h(x) for h in self.heads], dim=-1)
        out = self.proj(out) # Residual connections: 4
        out = self.dropout(out)  # Layernorm : 4
        return out

class FeedFoward(nn.Module): # Feedforward layers of transformer: 1
    """ a simple linear layer followed by a non-linearity"""

    def __init__(self, n_embd):
        super().__init__()
        self.net = nn.Sequential(
                nn.Linear(n_embd, 4 * n_embd), # Residual connections: 6
                nn.ReLU(),
                nn.Linear(4 * n_embd, n_embd), # Residual connections: 5 # Residual connections: 6
                nn.Dropout(dropout), # Layernorm : 5
        )

    def forward(self, x):
        return self.net(x)

class Block(nn.Module): # Residual connections: 1
    ...
    def __init__(self, n_embd, n_head):
        ...
        self.ln2 = nn.LayerNorm(n_embd) # Layernorm : 1

    def forward(self, x):
        x = x + self.sa(self.ln1(x)) # Residual connections: 3 # Layernorm : 1
        x = x + self.ffwd(self.ln2(x)) # Residual connections: 3 # Layernorm : 1
        return x
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self): # Minor code cleanup: 1
        ...
        #self.ffwd = FeedFoward(n_embd) # Feedforward layers of transformer: 2
        #self.blocks = nn.Sequential(
        #        Block(n_embd, n_head=4),
        #        Block(n_embd, n_head=4),
        #        Block(n_embd, n_head=4),
        #) # Residual connections: 2
        self.blocks = nn.Sequential(*[Block(n_embd, n_head=n_head) for _ in range(n_layer)]) # Layernorm : 3
        self.ln_f = nn.LayerNorm(n_embd) # final layer norm # Layernorm : 2
    ...
    def forward(self, idx, targets=None):
        ...
        #x = self.ffwd(x) # (B,T,C) # Feedforward layers of transformer: 2
        x = self.blocks(x) # (B,T,C) # Residual connections: 2
        x = self.ln_f(x) # (B,T,C) # Layernorm : 2
